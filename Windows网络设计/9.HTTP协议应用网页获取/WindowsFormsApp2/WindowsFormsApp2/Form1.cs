        ...
        {//窗体消息处理重载
            switch (m.Msg)
            {
                case TRAN_FINISHED:
                    ...
                    MemoryStream msDC = new MemoryStream(ms_recv.GetBuffer(), 20538 - 20265, recv_str_len);
                    GZipStream decompressdStream = new GZipStream(msDC, CompressionMode.Decompress);
                    ...
                    do
                    {
                        count = decompressdStream.Read(decompressedBuffer, readdcLen, 1000);
                        readdcLen = readdcLen + count;
                    }
                    while (count > 0);
                    recv_str = Encoding.UTF8.GetString(decompressedBuffer, 0, 20265);
